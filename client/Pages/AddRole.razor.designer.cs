...
namespace Hms.Pages
{
    public partial class AddRoleComponent : ComponentBase
    {
        ...
        {
            role = new Hms.Models.TelemetryServer.Role(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Hms.Models.TelemetryServer.Role args)
        {
            try
            {
                var telemetryServerCreateRoleResult = await TelemetryServer.CreateRole(role:role);
                DialogService.Close(role);
            }
            catch (System.Exception telemetryServerCreateRoleException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new Role!" });
            }
        }
