...
namespace Hms.Pages
{
    public partial class EditRoomComponent : ComponentBase
    {
        ...
        {
            var telemetryServerGetRoomByidResult = await TelemetryServer.GetRoomByid(id:id);
            room = telemetryServerGetRoomByidResult;
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Hms.Models.TelemetryServer.Room args)
        {
            try
            {
                var telemetryServerUpdateRoomResult = await TelemetryServer.UpdateRoom(id:id, room:room);
                DialogService.Close(room);
            }
            catch (System.Exception telemetryServerUpdateRoomException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update Room" });
            }
        }
