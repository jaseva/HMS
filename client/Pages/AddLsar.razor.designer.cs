...
namespace Hms.Pages
{
    public partial class AddLsarComponent : ComponentBase
    {
        ...
        {
            lsar = new Hms.Models.TelemetryServer.Lsar(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Hms.Models.TelemetryServer.Lsar args)
        {
            try
            {
                var telemetryServerCreateLsarResult = await TelemetryServer.CreateLsar(lsar:lsar);
                DialogService.Close(lsar);
            }
            catch (System.Exception telemetryServerCreateLsarException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new Lsar!" });
            }
        }
