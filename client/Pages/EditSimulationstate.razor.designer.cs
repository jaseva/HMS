...
namespace Hms.Pages
{
    public partial class EditSimulationstateComponent : ComponentBase
    {
        ...
        {
            var telemetryServerGetSimulationstateByidResult = await TelemetryServer.GetSimulationstateByid(id:id);
            simulationstate = telemetryServerGetSimulationstateByidResult;
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Hms.Models.TelemetryServer.Simulationstate args)
        {
            try
            {
                var telemetryServerUpdateSimulationstateResult = await TelemetryServer.UpdateSimulationstate(id:id, simulationstate:simulationstate);
                DialogService.Close(simulationstate);
            }
            catch (System.Exception telemetryServerUpdateSimulationstateException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update Simulationstate" });
            }
        }
