...
namespace Hms.Pages
{
    public partial class AddSimulationfailureComponent : ComponentBase
    {
        ...
        {
            simulationfailure = new Hms.Models.TelemetryServer.Simulationfailure(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Hms.Models.TelemetryServer.Simulationfailure args)
        {
            try
            {
                var telemetryServerCreateSimulationfailureResult = await TelemetryServer.CreateSimulationfailure(simulationfailure:simulationfailure);
                DialogService.Close(simulationfailure);
            }
            catch (System.Exception telemetryServerCreateSimulationfailureException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new Simulationfailure!" });
            }
        }
